...
from aio_pika.channel import Channel, ExchangeType
...
class PublishSubscribe(Base):
    ...
    def __init__(self, channel: Channel, exchange_name):
    ...
    def initialize_queue(self, **kwargs):
        self.queue = yield from self.channel.declare_queue(**kwargs)

    @asyncio.coroutine
    def initialize_exchange(self, **kwargs):
        self.exchange = yield from self.channel.declare_exchange(
            name=self.exchange_name, type=ExchangeType.FANOUT, **kwargs)
    ...
    def subscribe(self, func_name: str, func: Callable):
        """ Creates a new :class:`Worker` instance. """

        self.routes[func_name] = func

    @asyncio.coroutine
    def publish(self, func_name: str, kwargs=None):
        ...
        yield from self.exchange.publish(
