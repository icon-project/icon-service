...
class TestRcDataStorage:
    ...
    def test_rc_storage_check_data_format_by_revision(self,
                                                      context, rc_data_storage, dummy_header, dummy_gv,
                                                      revision,
                                                      expected_rc_db_version,
                                                      expected_header_revision,
                                                      expected_gv_config_main_prep_count,
    ...
    def test_when_initially_load_last_tx_index_should_return_minus_1(self, rc_data_storage):
    ...
    @pytest.mark.parametrize("block_height", [block_height for block_height in range(-2, 1)])
    def test_replace_db_invalid_block_height(self, rc_data_storage, block_height):
        with pytest.raises(AssertionError):
            rc_data_storage.replace_db(block_height)

    def test_commit_without_iiss_tx(self, dummy_header, dummy_gv, dummy_prep, rc_data_storage):
        ...
        assert rc_data_storage._db_iiss_tx_index == -1
        assert rc_data_storage._db.get(rc_data_storage.KEY_FOR_GETTING_LAST_TRANSACTION_INDEX) is None

    def test_commit_with_iiss_tx(self, dummy_header, dummy_gv, dummy_prep, dummy_tx, rc_data_storage):
        ...
        for expected_index in range(0, 10):
            ...
            iiss_wal: 'IissWAL' = IissWAL(dummy_iiss_data_list, rc_data_storage._db_iiss_tx_index, Revision.IISS.value)

            rc_data_storage.commit(iiss_wal)

            actual_recorded_index = int.from_bytes(
                rc_data_storage._db.get(rc_data_storage.KEY_FOR_GETTING_LAST_TRANSACTION_INDEX),
                'big')
            assert rc_data_storage._db_iiss_tx_index == actual_recorded_index == expected_index
