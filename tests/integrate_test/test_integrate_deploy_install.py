...
from tests import raise_exception_start_tag, raise_exception_end_tag, create_tx_hash, create_timestamp
...
class TestIntegrateDeployInstall(TestIntegrateBase):
    ...
    def _create_invalid_deploy_tx(self,
        ...
        deploy_params: dict = {}
        deploy_data: dict = {'contentType': 'application/zip', 'content': content, 'params': deploy_params}
    ...
    def test_duplicated_deploy_tx(self):
        ...
        prev_block, tx_results = self.make_and_req_block([tx], block_height=block_height)
        tx_results: List['TransactionResult'] = self.get_tx_results(hash_list)
        self.assertEqual(tx_results[0].status, int(True))
