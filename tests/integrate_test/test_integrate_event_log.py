...
class TestIntegrateEventLog(TestIntegrateBase):
    ...
    def test_event_log_self_is_not_defined(self):
        ...
        self.assertEqual(
            tx_results[0].failure.message,
            "'self' is not declared as the first parameter",
        )
    ...
    def test_event_log_when_error(self):
        ...
        expected = "default"
        actual = self.query_score(
            from_=self._accounts[0], to_=score_addr1, func_name="get_value"
        )
    ...
    def test_event_log_index_on_deploy(self):
        ...
        self.assertEqual(
            tx_results[0].failure.message, "Indexed arguments overflow: limit=3"
        )
        ...
        self.assertEqual(
            tx_results[0].failure.message, "Index exceeds the number of parameters"
        )
    ...
    def test_event_log_internal_call(self):
        ...
        tx_results: List["TransactionResult"] = self.score_call(
            ...
            params={
                "score_addr_b": str(score_addr_b),
                "score_addr_c": str(score_addr_c),
            },
        )
        event_log = tx_results[0].event_logs
        self.assertEqual(event_log[0].data[0], "A")
