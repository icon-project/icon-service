...
class TestIntegrateOnInstallParameters(TestIntegrateBase):
    ...
    def test_missing_parameters_oninstall(self):
        ...
        tx2 = self.create_deploy_score_tx(
            score_root="sample_deploy_scores",
            score_name=f"install/sample_token",
            from_=self._accounts[0],
            to_=SYSTEM_SCORE_ADDRESS,
        ...
        tx3 = self.create_deploy_score_tx(
            score_root="sample_deploy_scores",
            score_name=f"install/sample_token",
            from_=self._accounts[0],
            to_=SYSTEM_SCORE_ADDRESS,
        ...
        tx_results: List["TransactionResult"] = self.process_confirm_block_tx(
            [tx1, tx2, tx3], expected_status=False
        )
        ...
        self.assertTrue(
            tx_results[0].failure.message.find(
                "on_install() missing 1 required positional argument:"
            )
            != -1
        )
        self.assertTrue(
        ...
        self.assertTrue(
            tx_results[2].failure.message.find(
                "on_install() missing 2 required positional arguments:"
            )
            != -1
        )
    ...
    def test_invalid_parameter_value_oninstall(self):
        tx1 = self.create_deploy_score_tx(
            ...
            to_=SYSTEM_SCORE_ADDRESS,
            deploy_params={
                "init_supply": str(self._accounts[0].address),
                "decimal": hex(18),
            },
        )
