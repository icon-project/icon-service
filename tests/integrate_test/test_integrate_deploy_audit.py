...
class TestIntegrateDeployAudit(TestIntegrateBase):
    ...
    def test_governance_call_about_add_auditor_already_auditor_update_governance(self):
        ...
        self.assertEqual(
            tx_results[0].failure.message, "Invalid address: already auditor"
        )
    ...
    def test_governance_call_about_add_remove_auditor_score_addr_update_governance(
        ...
        self.assertEqual(tx_results[0].failure.code, ExceptionCode.SCORE_ERROR)
        self.assertEqual(
            tx_results[0].failure.message, f"Invalid EOA Address: {str(score_addr)}"
        )
    ...
    def test_builtin_score(self):
        expect_ret = {"current": {"status": "active"}}
    ...
    def test_builtin_score_update_governance(self):
        ...
        expect_ret = {
            "current": {"status": "active", "deployTxHash": tx_results[0].tx_hash}
        }
        self._assert_get_score_status(GOVERNANCE_SCORE_ADDRESS, expect_ret)
    ...
    def test_normal_score_fail2(self):
        ...
        # 3. update (wait audit)
        tx_results: List["TransactionResult"] = self._deploy_score(
            "update/sample_score", 2, to_=score_addr1
        )
        ...
        expect_ret = {
            ...
            "next": {"status": "pending", "deployTxHash": tx_hash3},
        }
        self._assert_get_score_status(score_addr1, expect_ret)

        # 4. overwrite
        tx_results: List["TransactionResult"] = self._deploy_score(
            "update/sample_score", 3, to_=score_addr1
        )
        ...
        expect_ret = {
            ...
            "next": {"status": "pending", "deployTxHash": tx_hash4},
        }
        self._assert_get_score_status(score_addr1, expect_ret)
    ...
    def test_normal_score_fail4_fix_update_governance_1_0_0(self):
        ...
        tx_results: List["TransactionResult"] = self.accept_score(
            tx_hash1, expected_status=False
        )
        self.assertEqual(
            tx_results[0].failure.message, "Invalid txHash: already accepted"
        )
