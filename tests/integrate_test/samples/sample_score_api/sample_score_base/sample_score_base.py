...
class SampleScoreBase(IconScoreBase):
    ...
    def test_create_interface_score_readonly(self, address: Address) -> bool:
        return_value = self.create_interface_score(address, SampleScoreBaseInterface)
        assert type(type(return_value)) == type(SampleScoreBaseInterface)
        return True

    @external
    def test_create_interface_score(self, address: Address):
        self.create_interface_score(address, SampleScoreBaseInterface)
        self._value.set(10 ** 18)
        self.Changed(10 ** 18)
