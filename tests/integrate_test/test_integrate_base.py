...
from iconservice.iiss.reward_calc.ipc.reward_calc_proxy import (
    RewardCalcProxy,
...
class TestIntegrateBase(TestCase):
    ...
    def mock_calculate(self, _path, _block_height):
        ...
        response = CalculateDoneNotification(
            0, True, end_block_height_of_calc - calc_period, 0, b"mocked_response"
        )
    ...
    def _mock_ipc(cls, mock_calculate: callable = mock_calculate):
        ...
        RewardCalcProxy.query_calculate_result = Mock(
            return_value=(RCCalculateResult.SUCCESS, 0, 0, bytes())
        )
    ...
    def _genesis_invoke(self) -> tuple:
        ...
        invoke_response: tuple = self.icon_service_engine.invoke(block, [tx])
    ...
    def make_and_req_block(
        ...
        (
            ...
            added_transactions,
            main_prep_as_dict,
        ) = self.icon_service_engine.invoke(
            block=block,
            tx_requests=tx_list,
    ...
    def debug_make_and_req_block(
        ...
        if block is None:
            ...
            timestamp_us = create_timestamp()
            block = Block(
                block_height, block_hash, timestamp_us, self._prev_block_hash, 0
            )
    ...
    def create_score_call_tx(
        ...
        request_params = {
            ...
            "signature": self._signature,
            "dataType": "call",
            "data": {"method": func_name, "params": params},
        }
    ...
    def create_transfer_icx_tx(
        ...
        ] = self._convert_address_from_address_type(to_)
    ...
    def create_register_proposal_tx(
        self,
        from_: "Address",
        title: str,
        description: str,
    ...
    def create_vote_proposal_tx(
        ...
        method = "voteProposal"
        score_params = {"id": bytes_to_hex(id_, "0x"), "vote": hex(vote)}
    ...
    def update_governance(
        ...
        expected_status: bool = True,
        root_path: str = "sample_builtin",
    ) -> List["TransactionResult"]:

        tx = self.create_deploy_score_tx(
            root_path, f"{version}/governance", self._admin, GOVERNANCE_SCORE_ADDRESS
        )
        return self.process_confirm_block_tx([tx], expected_status)

    def update_governance_for_audit(
        self,
        version: str = "latest_version",
        expected_status: bool = True,
        root_path: str = "sample_builtin",
    ...
    def set_revision(
        self, revision: int, expected_status: bool = True
    ) -> List["TransactionResult"]:
    ...
    def deposit_icx(
        ...
        if sender is None:
            sender = self._admin
        if FIXED_TERM:
            tx: dict = self.create_deposit_tx(
                from_=sender, to_=score_address, action="add", params={}, value=amount
            )
        else:
            tx: dict = self.create_deposit_tx(
                from_=sender,
    ...
    def register_proposal(
        ...
        tx: dict = self.create_register_proposal_tx(
            from_, title, description, type_, value
        )
    ...
    def vote_proposal(
        self, from_: "Address", id_: bytes, vote: bool, expected_status: bool = True
    ) -> List["TransactionResult"]:
        tx: dict = self.create_vote_proposal_tx(from_, id_, vote)
        return self.process_confirm_block_tx([tx], expected_status)

    def get_balance(self, account: Union["EOAAccount", "Address"]) -> int:

        address: Optional["Address"] = self._convert_address_from_address_type(account)

        return self._query(request={"address": address}, method="icx_getBalance")

    def get_total_supply(self) -> int:
        return self._query(request={}, method="icx_getTotalSupply")

    def get_score_api(self, address: "Address"):
        return self._query(request={"address": address}, method="icx_getScoreApi")
    ...
    def query_score(
        ...
        query_request = {
            ...
            "to": to_,
            "dataType": "call",
            "data": {"method": func_name, "params": {} if params is None else params},
        }
    ...
    def get_score_status(self, to_: "Address"):
        query_request = {
            ...
            "dataType": "call",
            "data": {"method": "getScoreStatus", "params": {"address": str(to_)}},
        }
        return self._query(query_request)
    ...
    def get_step_price(self) -> int:
        query_request = {
            ...
            "to": GOVERNANCE_SCORE_ADDRESS,
            "dataType": "call",
            "data": {"method": "getStepPrice", "params": {}},
        }
        return self._query(query_request)
