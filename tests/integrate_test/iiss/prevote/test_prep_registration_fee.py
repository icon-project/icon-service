...
class TestIntegratePrepRegistration(TestIISSBase):
    ...
    def test_register_prep_with_invalid_icx_value(self):
        ...
        self.register_prep(from_=self._accounts[0], value=0, expected_status=False)
        ...
        self.register_prep(
            from_=self._accounts[0], value=insufficient_value, expected_status=False
        )

        excess_value = 2500 * ICX_IN_LOOP
        self.register_prep(
            from_=self._accounts[0], value=excess_value, expected_status=False
        )

    def test_register_prep_burn_event_log_should_be_fixed_after_revision_9(self):
        preps: list = self.create_eoa_accounts(3)
        self.distribute_icx(accounts=preps, init_balance=3000 * ICX_IN_LOOP)
        ...
        reg_tx_result: "TransactionResult" = self.register_prep(
            from_=prep, value=config_value
        )[0]
    ...
    def test_register_prep_burn_signature(self):
        ...
        for i in range(PREP_MAIN_PREPS):
            ...
            self.assertEqual(
                icx_amount_before_reg - expected_burned_amount - step_price,
    ...
    def test_set_value_when_prep_related_set_method(self):
        ...
        # register prep
        self.register_prep(
            from_=self._accounts[0], value=self._config[ConfigKey.PREP_REGISTRATION_FEE]
        )
