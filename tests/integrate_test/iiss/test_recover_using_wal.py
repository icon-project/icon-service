...
class TestRecoverUsingWAL(TestIISSBase):
    ...
    def _get_precommit_data_after_invoke(self) -> "PrecommitData":
        ...
        precommit_data = self.icon_service_engine._get_updated_precommit_data(
            block.hash, block.hash
        )
        return precommit_data

    def _get_wal_writer(
        self, precommit_data: "PrecommitData", is_calc_period_start_block: bool
    ):
    ...
    def _check_the_state_and_rc_db_after_recover(
        ...
        self.assertEqual(
            unregister_status, self.get_prep(self.prep_to_be_unregistered)["status"]
        )
        ...
        for rc_data in current_rc_db.iterator():
            ...
            if (
                is_calc_period_start_block
                and rc_data[0] == RewardCalcStorage.KEY_FOR_VERSION_AND_REVISION
            ):
            ...
            if (
                is_calc_period_start_block
                and rc_data[0][:2] == GovernanceVariable.PREFIX
            ):
    ...
    def _check_the_db_after_recover(
        self, last_block_before_close: int, is_calc_period_start_block: bool
    ):
    ...
    def _remove_all_iiss_db_before_reopen(self):
        ...
        for dir_name in os.listdir(self.rc_data_path):
            if dir_name.startswith(RewardCalcStorage.IISS_RC_DB_NAME_PREFIX):
                shutil.rmtree(
                    os.path.join(self.rc_data_path, dir_name),
                    ignore_errors=False,
                    onerror=None,
                )
    ...
    def test_close_during_writing_rc_db_on_calc_period(self):
        ...
        self.icon_service_engine._process_iiss_commit(
    ...
    def test_close_before_sending_calculate_on_the_start(self):
        ...
        self.icon_service_engine._process_state_commit(
            context, precommit_data, state_wal
        )
