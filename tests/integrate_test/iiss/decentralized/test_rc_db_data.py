...
class TestRCDatabase(TestIISSBase):
    ...
    def get_last_rc_db_data(rc_data_path):
        return sorted(
            [
                dir_name
                for dir_name in os.listdir(rc_data_path)
                if dir_name.startswith(RewardCalcStorage.IISS_RC_DB_NAME_PREFIX)
            ],
            key=lambda rc_dir: int(
                rc_dir[len(RewardCalcStorage.IISS_RC_DB_NAME_PREFIX) + 1 :]
            ),
            reverse=True,
        )[0]
    ...
    def test_all_rc_db_data_block_height(self):
        ...
        for rc_data in rc_db.iterator():
            ...
            if rc_data[0][:2] == Header.PREFIX:
                ...
                self.assertEqual(
                    expected_block_height_at_the_start_of_iiss, hd.block_height
                )
            rc_data_count += 1
        expected_rc_db_data_count: int = 1
        ...
        # distribute icx PREP_MAIN_PREPS ~ PREP_MAIN_PREPS + PREP_MAIN_PREPS - 1
        self.distribute_icx(
            accounts=self._accounts[:PREP_MAIN_PREPS], init_balance=10 * ICX_IN_LOOP
        )
    ...
    def test_change_revision(self):
        ...
        for rc_data in rc_db.iterator():
            if rc_data[0][:2] == PRepsData.PREFIX:
                ...
                diff_cnt: int = 0
                self.assertEqual(
                    diff_cnt, len(set(expected_prep_address) ^ set(prep_addresses))
                )
            if rc_data[0][:2] == Header.PREFIX:
                hd: "Header" = Header.from_bytes(rc_data[1])
                expected_version = 2
        ...
        for rc_data in rc_db.iterator():
            if rc_data[0][:2] == PRepsData.PREFIX:
                ...
                self.assertEqual(expected_prep_block, preps.block_height)
                prep_addresses: list = [
                    delegation_info.address for delegation_info in preps.prep_list
                ]
    ...
    def test_bp_on_the_start_term(self):
        ...
        for rc_data in iiss_db.iterator():
            if (
                rc_data[0][:2] == BlockProduceInfoData.PREFIX
                and int.from_bytes(rc_data[0][2:], "big") == calc_end_block
            ):
