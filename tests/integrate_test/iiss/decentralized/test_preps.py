...
class TestPreps(TestIISSBase):
    ...
    def test_prep_rotate(self):
        ...
        """
        self.distribute_icx(
            accounts=self._accounts[:PREP_MAIN_AND_SUB_PREPS],
        ...
        self.distribute_icx(
            accounts=self._accounts[PREP_MAIN_PREPS : PREP_MAIN_PREPS + 1],
            init_balance=20000,
        )

        self.set_stake(from_=self._accounts[PREP_MAIN_PREPS], value=10000)
        ...
        self.set_delegation(
            from_=self._accounts[index],
            origin_delegations=[(self._accounts[index], delegation_amount)],
        )
        ...
        for i in range(1, PREP_MAIN_PREPS):
            expected_preps.append(
                {"address": self._accounts[i].address, "delegated": 0}
            )
        expected_preps.insert(
            0,
            {
    ...
    def test_set_governance_variables1(self):
        ...
        self.distribute_icx(
            accounts=self._accounts[:1], init_balance=10000 * ICX_IN_LOOP
        )

        origin_irep: int = IISS_INITIAL_IREP
        tx: dict = self.create_set_governance_variables(
            from_=self._accounts[0], irep=origin_irep
        )
        ...
        self.assertEqual(
            expected_update_block_height, response["irepUpdateBlockHeight"]
        )
    ...
    def test_set_governance_variables3(self):
        ...
        for i in range(possible_maximum_raise_irep_count):
            ...
            irep: int = irep * 12 // 10
            tx: dict = self.create_set_governance_variables(
                from_=self._accounts[0], irep=irep
            )
            self.process_confirm_block_tx([tx])
    ...
    def test_weighted_average_of_irep(self):
        self.distribute_icx(
            ...
            + self._accounts[PREP_MAIN_PREPS : PREP_MAIN_PREPS + 2],
            init_balance=1 * ICX_IN_LOOP,
        )

        tx_list = []
        for account in self._accounts[PREP_MAIN_PREPS : PREP_MAIN_PREPS + 2]:
            tx = self.create_set_stake_tx(from_=account, value=3)
            tx_list.append(tx)
        ...
        tx1: dict = self.create_set_delegation_tx(
            ...
            origin_delegations=[(self._accounts[0], delegation1)],
        )
        delegation2: int = 3
        tx2: dict = self.create_set_delegation_tx(
            from_=self._accounts[PREP_MAIN_PREPS + 1],
            origin_delegations=[(self._accounts[1], delegation2)],
        )
    ...
    def test_register_prep_apply_terms_irep(self):
        ...
        irep1: int = IISS_INITIAL_IREP * 12 // 10
        tx1: dict = self.create_set_governance_variables(
            from_=self._accounts[0], irep=irep1
        )
        irep2: int = IISS_INITIAL_IREP * 8 // 10
        tx2: dict = self.create_set_governance_variables(
            from_=self._accounts[1], irep=irep2
        )
        self.process_confirm_block_tx([tx1, tx2])
    ...
    def test_get_prep_term(self):
        ...
        for i, account in enumerate(accounts[:PREP_MAIN_PREPS]):
            ...
            self.assertEqual(
                prep[ConstantKeys.P2P_ENDPOINT], data[ConstantKeys.P2P_ENDPOINT]
            )

        # distribute icx for register
        self.distribute_icx(
            accounts=accounts[PREP_MAIN_PREPS:PREP_MAIN_AND_SUB_PREPS],
