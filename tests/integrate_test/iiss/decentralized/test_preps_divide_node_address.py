...
class TestPRepNodeAddressDivision(TestIISSBase):
    ...
    def test_prep_register_node_address_before_rev_DIVIDE_NODE_ADDRESS(self):
        ...
        self.distribute_icx(accounts=[account], init_balance=1 * ICX_IN_LOOP)
        ...
        tx: dict = self.create_register_prep_tx(from_=account, reg_data=reg_data)

        _, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=[tx]
        )
    ...
    def test_prep_set_node_address_before_rev_DIVIDE_NODE_ADDRESS(self):
        ...
        tx: dict = self.create_set_prep_tx(
            from_=account, set_data={"nodeAddress": str(dummy_node)}
        )
    ...
    def test_prep_register_node_address(self):
        ...
        block, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=[tx]
        )
    ...
    def test_prep_set_node_address_check_generator(self):
        ...
        for i in range(dummy_cnt):
            tx_list.append(
                self.create_set_prep_tx(
                    from_=account, set_data={"nodeAddress": str(dummy_addrs[i])}
                )
            )
        ...
        prev_block_votes = [
            (x.address, True) for x in self._accounts[1:PREP_MAIN_PREPS]
        ]
        block, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=[],
        ...
        # set prep 2
        tx: dict = self.create_set_prep_tx(
            from_=account, set_data={"nodeAddress": str(dummy_node2)}
        )
    ...
    def test_scenario1(self):
        ...
        tx_list: list = [
            self.create_set_prep_tx(from_=prep_b, set_data={"nodeAddress": str(prep_a)})
        ]
        ...
        self.assertEqual(
            tx_results[1].failure.message, f"nodeAddress already in use: {str(prep_a)}"
        )
    ...
    def test_scenario2(self):
        ...
        tx_list: list = [
            self.create_set_prep_tx(
                from_=prep_a, set_data={"nodeAddress": str(new_addr)}
            )
        ]
        block, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=tx_list
        ...
        tx_list: list = [
            self.create_set_prep_tx(
                from_=prep_a, set_data={"nodeAddress": str(old_addr)}
            )
        ]
        block, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=tx_list
    ...
    def test_scenario3(self):
        ...
        tx_list: list = [
            self.create_set_prep_tx(
                from_=prep_a, set_data={"nodeAddress": str(prep_a_new_addr1)}
            )
        ]
        block, tx_results, _, _, main_prep_as_dict = self.debug_make_and_req_block(
            tx_list=tx_list
        ...
        tx_list: list = [
            self.create_set_prep_tx(
                from_=prep_a, set_data={"nodeAddress": str(prep_a_new_addr2)}
            ),
            self.create_set_prep_tx(
                from_=prep_b, set_data={"nodeAddress": str(prep_a_new_addr1)}
            ),
        ]
    ...
    def test_scenario4(self):
        ...
        tx_list: list = [
            self.create_unregister_prep_tx(from_=prep_b),
            self.create_set_prep_tx(
                from_=prep_a, set_data={"nodeAddress": str(prep_b.address)}
            ),
        ]
    ...
    def test_scenario5(self):
        ...
        tx_results = self.make_blocks(
            to=self._block_height + 2,
            prev_block_generator=self._accounts[0].address,
