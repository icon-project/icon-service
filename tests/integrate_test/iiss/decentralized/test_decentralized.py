...
class TestIISSDecentralized(TestIISSBase):
    ...
    def test_overlapped_block(self):
        ...
        block, var1_1, var1_2, var1_3, var1_4 = self.debug_make_and_req_block(
            tx_list=[]
        )
        _, var2_1, var2_2, var2_3, var2_4 = self.debug_make_and_req_block(
            tx_list=[], block=block
        )
    ...
    def test_estimate_step(self):
        ...
        tx = self.create_transfer_icx_tx(
            from_=self._admin, to_=self._accounts[prep_id], value=balance
        )
        self.process_confirm_block_tx([tx])

        # set stake
        tx: dict = self.create_set_stake_tx(from_=self._accounts[prep_id], value=0)
        self.estimate_step(tx)

        # set delegation
        tx: dict = self.create_set_delegation_tx(
            from_=self._accounts[prep_id],
        ...
        tx: dict = self.create_set_governance_variables(
            from_=self._accounts[prep_id], irep=IISS_MIN_IREP
        )
        self.estimate_step(tx)
    ...
    def test_irep_each_revision(self):
        ...
        for i in range(PREP_MAIN_PREPS):
            tx: dict = self.create_transfer_icx_tx(
                from_=self._admin, to_=self._accounts[i], value=3000 * ICX_IN_LOOP
            )
            tx_list.append(tx)
        self.process_confirm_block_tx(tx_list)
