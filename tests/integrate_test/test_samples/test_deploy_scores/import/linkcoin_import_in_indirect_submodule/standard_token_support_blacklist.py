...
def only_operator(func):
    if not isfunction(func):
        raise IconServiceBaseException(f"{func} isn't function.")
    ...
    def __wrapper(calling_obj: object, *args, **kwargs):
        if isinstance(calling_obj, StandardTokenSupportBlacklist):
            ...
            raise IconServiceBaseException(f"{sender} don't have authority'")
        raise IconServiceBaseException(f"invalid operation")
