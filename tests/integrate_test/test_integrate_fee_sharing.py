...
class TestIntegrateFeeSharing(TestIntegrateBase):
    ...
    def _query_score_info(self, address: Address):
        query_request = {
            ...
            "dataType": "call",
            "data": {"method": "getScoreStatus", "params": {"address": str(address)}},
        }
    ...
    def test_deposit_fee(self):
        ...
        self.assertIn(
            deposit_id, map(lambda d: d["id"], score_info["depositInfo"]["deposits"])
        )
        after_balance: int = self.get_balance(self._admin)

        self.assertEqual(
            before_balance - MIN_DEPOSIT_AMOUNT - deposit_fee, after_balance
        )
    ...
    def test_deposit_fee_eventlog(self):
        ...
        self.assertEqual(
            step_used_before_iiss_rev, step_used_after_iiss_rev + event_log_fee
        )
    ...
    def test_sharing_fee_case_score_50(self):
        ...
        user_used_fee: int = tx_results[0].step_used_details[admin_addr] * tx_results[
            0
        ].step_price
        score_used_fee: int = tx_results[0].step_used_details[
            self.score_address
        ] * tx_results[0].step_price
        after_call_user_balance: int = self.get_balance(self._admin)
        ...
        self.assertEqual(
            initial_available_deposit - remaining_step, after_call_available_deposit
        )
    ...
    def test_score_call_after_deposit_expired(self):
        # deposit icx
        self.deposit_icx(
            score_address=self.score_address, amount=3 * MIN_DEPOSIT_AMOUNT, period=1
        )
        ...
        for i in range(MIN_DEPOSIT_TERM):
            self.transfer_icx(from_=self._admin, to_=self._accounts[0], value=10 ** 8)
    ...
    def test_deposit_unauthorized_account(self):
        # give icx to tester
        self.transfer_icx(
            from_=self._admin, to_=self._accounts[0], value=10000 * ICX_IN_LOOP
        )
    ...
    def test_deposit_nonexistent_score(self):
        ...
        with self.assertRaises(InvalidRequestException) as e:
            self.deposit_icx(
                score_address=Address.from_prefix_and_int(AddressPrefix.CONTRACT, 3),
                amount=MIN_DEPOSIT_AMOUNT,
                period=MIN_DEPOSIT_TERM,
            )
    ...
    def test_get_score_info_with_deposits(self):
        ...
        self.assertEqual(
            deposit_info["availableVirtualStep"],
        ...
        for i in range(len(deposit_info["deposits"])):
            sum_of_available_deposit += (
                deposit_info["deposits"][i]["depositAmount"] * 90 // 100
            )
    ...
    def test_withdraw_deposit_after_deposit(self):
        ...
        self.assertEqual(
            "DepositWithdrawn(bytes,Address,int,int)", event_log.indexed[0]
        )
    ...
    def test_withdraw_deposit_again_after_already_withdraw_one(self):
        ...
        # withdraw
        self.withdraw_deposit(deposit_id=deposit_id, score_address=self.score_address)
    ...
    def test_inter_call_fee_sharing_proportion100(self):
        # deposit icx
        self.deposit_icx(
            score_address=self.score_address2,
            amount=3 * MIN_DEPOSIT_AMOUNT,
            period=MIN_DEPOSIT_TERM,
        )
        user_balance: int = self.get_balance(self._admin)
