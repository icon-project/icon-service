...
class TestICONSCOREINSTALLER(unittest.TestCase):
    def setUp(self):
        ...
        self.address2 = Address.from_string("cx1234123412341234123412341234123412341235")
        self.score_root_path2 = os.path.join(self.installer.icon_score_root_path, str(self.address2))
    ...
    def test_install(self):
        ...
        self.installer.install(self.address2, self.installer.read_zipfile_as_byte(self.archive_path)
                               , block_height1, transaction_index1)
        install_path = os.path.join(self.score_root_path2, score_id)
        self.assertEqual(True, os.path.exists(install_path))

        ret1 = self.installer.install(self.address2, self.installer.read_zipfile_as_byte(self.archive_path)
                                      , block_height1, transaction_index1)
        self.assertEqual(CONST_SCORE_EXISTS_ERROR_CODE, ret1)

        self.installer.remove_exists_archive(os.path.join('./', str(self.address2)))

        ret2 = self.installer.install(self.address2, self.installer.read_zipfile_as_byte(self.archive_path2)
                                      , block_height1, transaction_index1)
