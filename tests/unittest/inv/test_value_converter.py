...
class TestValueConverter:
    @pytest.mark.parametrize(
        "inv_type, inv_value, expected_inv_value",
        [
    ...
    @pytest.mark.parametrize(
        "invalid_max_step_limits",
        [{"direct": 10}, {"estimation": 10}, {1: 10}, {b"bytes": 10}],
    )
    def test_when_convert_invalid_convert_max_step_limits_should_raise_exception(
        self, invalid_max_step_limits
    ):
        with pytest.raises(BaseException):
            ValueConverter.convert_for_icon_service(
                IconNetworkValueType.MAX_STEP_LIMITS, invalid_max_step_limits
            )

    def test_when_convert_unknown_step_type_key_should_pass(self):
        step_costs = {"default": 10, "contractCall": 10, "unknown_type": 0}
        expected_step_costs = {
            StepType("default"): 10,
            StepType("contractCall"): 10,
        }

        converted_step_costs = ValueConverter.convert_for_icon_service(
            IconNetworkValueType.STEP_COSTS, step_costs
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                IconNetworkValueType.MAX_STEP_LIMITS,
                {IconScoreContextType.INVOKE: 10},
                {"invoke": 10},
            ),
    ...
    def test_convert_for_governance(self, inv_type, inv_value, expected_inv_value):
        actual_converted_value: Any = ValueConverter.convert_for_governance(
            inv_type, inv_value
        )
