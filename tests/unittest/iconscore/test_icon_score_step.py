...
class TestGetInputDataSize:
    ...
    @pytest.mark.parametrize(
        "revision", [r.value for r in Revision if r.value < Revision.THREE.value]
    )
    @pytest.mark.parametrize(
        "input_data, expected_call_data", [(x, [mock.call(x)]) for x in INPUT]
    )
    def test_less_than_revision_three(
        self,
        mock_get_data_size_recursively,
        mock_get_data_size_using_json_dumps,
        revision,
        input_data,
        expected_call_data,
    ):
    ...
    def test_greater_than_revision_three(
        self,
        mock_get_data_size_recursively,
        mock_get_data_size_using_json_dumps,
        revision,
...
class TestGetDeployContentSize:
    ...
    def test_get_deploy_content_size_less_than_revision_three(
        self, mock_get_data_size_recursively, revision, input_data
    ):
    ...
    def test_valid_get_deploy_content_size_greater_than_revision_three(
        self, mock_get_data_size_recursively, revision, input_data
    ):
    ...
    def test_invalid_get_deploy_content_size_greater_than_revision_three(
        self, mock_get_data_size_recursively, revision, input_data
    ):
...
class TestIconScoreStepCounter:
    ...
    def mock_step_counter_for_apply_step(self, mock_step_counter):
        def _data(external_call_count, step_costs_get_return_value):
            ...
            mock_step_counter._step_costs.get = mock.Mock(
                return_value=step_costs_get_return_value
            )
    ...
    @pytest.mark.parametrize(
        ...
        [MAX_EXTERNAL_CALL_COUNT - 1, MAX_EXTERNAL_CALL_COUNT],
    )
    ...
    def test_apply_step(
        ...
        mock_step_counter_for_apply_step = mock_step_counter_for_apply_step(
            external_call_count=mock_step_counter_external_call_count,
            step_costs_get_return_value=mock_step_counter_step_costs_return_value,
        )

        if (
            step_type == StepType.CONTRACT_CALL
            and mock_step_counter_external_call_count > MAX_EXTERNAL_CALL_COUNT - 1
        ):
            # raise exception
            with pytest.raises(InvalidRequestException) as e:
            ...
            expected_step = (
                mock_step_counter_for_apply_step._step_costs.get(step_type, 0) * count
            )
            mock_step_counter_for_apply_step.apply_step(step_type, count)
            mock_step_counter_for_apply_step.consume_step.assert_called_once_with(
                step_type, expected_step
            )
    ...
    @pytest.mark.parametrize(
        "mock_step_counter_step_used", {PIVOT_VALUE - 1, PIVOT_VALUE, PIVOT_VALUE + 1}
    )
    @pytest.mark.parametrize(
        "mock_step_counter_step_limit", {PIVOT_VALUE - 1, PIVOT_VALUE, PIVOT_VALUE + 1}
    )
    @pytest.mark.parametrize(
        "mock_step_counter_max_step_used",
        {PIVOT_VALUE - 1, PIVOT_VALUE, PIVOT_VALUE + 1},
    )
    @pytest.mark.parametrize("step_type", [t for t in StepType])
    @pytest.mark.parametrize("step", [0, 2])
    def test_consume_step(
        self,
        ...
        mock_step_counter_for_consume_step = mock_step_counter_for_consume_step(
            max_step_used=mock_step_counter_max_step_used,
        ...
        if expected_step_used > mock_step_counter_step_limit:
            with pytest.raises(OutOfStepException) as e:
                mock_step_counter_for_consume_step.consume_step(
            ...
            step_used = mock_step_counter_for_consume_step.consume_step(
                step_type=step_type, step=step
            )
            mock_step_counter_for_consume_step._trace_step.assert_called_once_with(
                step_type, step
            )
