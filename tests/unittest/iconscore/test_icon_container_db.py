...
class TestIconContainerDB:
    ...
    @pytest.mark.parametrize(
        "args, value_type, expected_value",
        [
            (0, int, 1),
            (1, int, 2),
            (2, int, 3),
    ...
    def test_nested_list(self, score_db, args, value_type, expected_value):
        ...
        if isinstance(args, tuple):
            assert (
                ContainerUtil.get_from_db(
                    score_db, "test_list", *args, value_type=value_type
                )
                == expected_value
            )
        else:
    ...
    def test_nested_dict(self, score_db, args, value_type, expected_value):
        ...
        if isinstance(args, tuple):
            assert (
                ContainerUtil.get_from_db(
                    score_db, "test_dict", *args, value_type=value_type
                )
                == expected_value
            )
        else:
    ...
    @pytest.mark.parametrize(
        "args, value_type, expected_value",
        [(0, int, 1), (1, int, 2), (2, int, 3), (3, Address, ADDRESS),],
    )
    def test_tuple(self, score_db, args, value_type, expected_value):
        ...
        assert (
            ContainerUtil.get_from_db(
                score_db, "test_tuple", args, value_type=value_type
            )
            == expected_value
        )
    ...
    @pytest.mark.parametrize(
        ...
        [({"dummy_key": "dummy_value"}, "not_exists_key"), (["dummy_list"], 3)],
    )
    @pytest.mark.parametrize(
        "value_type, expected_value",
        [(int, 0), (str, ""), (bytes, None), (Address, None)],
    )
    def test_default_value_of_container_db(
        self, score_db, value_type, expected_value, collection, key_or_index
    ):
        # TEST: Check the default value of collection object (dict, list)
        ContainerUtil.put_to_db(score_db, "test_collection", collection)
        actual_value = ContainerUtil.get_from_db(
            score_db, "test_collection", key_or_index, value_type=value_type
        )
    ...
    @pytest.mark.parametrize(
        "prefix, score_db_cls, expected_prefix",
        [("a", ArrayDB, b"\x00|a"), ("dictdb", DictDB, b"\x01|dictdb"),],
    )
