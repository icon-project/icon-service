...
class TestExternalPayableCall:
    @pytest.mark.parametrize(
        "context_type, func_type, msg_value, func_name, args, kwargs",
        [
            (
                IconScoreContextType.INVOKE,
                IconScoreFuncType.READONLY,
                0,
                "func1",
                (),
                {},
    ...
    def test_external_call(
        self, context, context_type, func_type, msg_value, func_name, args, kwargs
    ):
    ...
    def test_call_with_value_to_not_payable_call_raise_exception(
        self, context, context_type, func_type, msg_value, func_name, args, kwargs
    ):
        context.context_type = context_type
        context.func_type = func_type
    ...
    @pytest.mark.parametrize(
        "context_type", [context_type for context_type in IconScoreContextType]
    )
    @pytest.mark.parametrize(
        "func_type", [func_type for func_type in IconScoreFuncType]
    )
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                marks=pytest.mark.xfail(
                    raises=MethodNotFoundException, reason="Method does not exists"
                ),
            ),
        ],
    )
    def test_inherit_call_case(
        self, context, context_type, func_type, msg_value, func_name, args, kwargs
    ):
        context.context_type = context_type
        context.func_type = func_type
