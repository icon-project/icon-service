...
class TestIconScoreDeployer(unittest.TestCase):
    ...
    def test_install(self):
        self.normal_score_path = os.path.join(
            DIRECTORY_PATH, "sample", "normal_score.zip"
        )
        self.bad_zip_file_path = os.path.join(
            DIRECTORY_PATH, "sample", "badzipfile.zip"
        )
        ...
        score_deploy_path: str = get_score_deploy_path(
            self.score_root_path, self.address, tx_hash1
        )

        IconScoreDeployer.deploy(
            score_deploy_path, self.read_zipfile_as_byte(self.normal_score_path)
        )
        ...
        score_deploy_path: str = get_score_deploy_path(
            self.score_root_path, self.address, tx_hash2
        )

        with self.assertRaises(BaseException) as e:
            IconScoreDeployer.deploy(
                score_deploy_path, self.read_zipfile_as_byte(self.bad_zip_file_path)
            )
        ...
        score_deploy_path: str = get_score_deploy_path(
            self.score_root_path, self.address, tx_hash3
        )
        IconScoreDeployer.deploy(
    ...
    def test_remove_existing_score(self):
        ...
        score_deploy_path: str = get_score_deploy_path(
            self.score_root_path, self.address, tx_hash
        )
    ...
    def test_deploy_when_score_depth_is_different(self):
        ...
        for zip_item in zip_list:
            ...
            IconScoreDeployer.deploy(
                score_deploy_path, self.read_zipfile_as_byte(self.archive_path)
            )
            self.assertEqual(True, os.path.exists(score_deploy_path))
    ...
    def test_deploy_bug_IS_355(self):
        ...
        for revision, expected_list in zip_list:
            ...
            self.archive_path = os.path.join(
                DIRECTORY_PATH, "sample", "score_registry.zip"
            )
    ...
    def test_deploy_when_score_depth_is_different_above_revision3(self):
        ...
        zip_list = [
            ...
            ("nodir.zip", ["__init__.py", "package.json", "sample_token.py"]),
        ]
