...
def mock_score_invoke(context, to, data_type, data):
    EventLogEmitter.emit_event_log(
        ...
        [Address.from_string(f"hx{'1'*40}"), Address.from_string(f"hx{'2'*40}"), 100],
        2,
    )
...
class TestFeeSharing(unittest.TestCase):
    ...
    def test_transaction_result_on_sharing_fee_user_ratio50(self, score_invoke):
        ...
        IconScoreContext.engine.fee.charge_transaction_fee = Mock(
            return_value={self.from_: 9000, self.score: 9000}
        )
        tx_hash = bytes.hex(os.urandom(32))

        data = {
            "method": "transfer",
            "params": {"to": f"hx{'2'*40}", "amount": hex(100)},
        }
    ...
    @patch(
        "iconservice.iconscore.icon_score_context_util.IconScoreContextUtil.is_service_flag_on"
    )
