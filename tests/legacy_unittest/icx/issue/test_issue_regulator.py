...
class TestIssueRegulator:
    ...
    def test_reflect_difference_in_issuing(self):
        ...
        for cumulative_fee in range(0, 2005):
            (
                actual_covered_by_fee,
                actual_covered_by_remain,
                actual_remain_over_issued_icx,
                actual_corrected_icx_issue_amount,
            ) = Regulator._reflect_difference_in_issuing(
                icx_issue_amount, over_issued_icx, cumulative_fee
            )
            expected_cumulative_fee = (
                0
                if cumulative_fee <= max_covered_by_fee
                else cumulative_fee - max_covered_by_fee
            )
            assert (
                actual_covered_by_fee == cumulative_fee
                if cumulative_fee <= max_covered_by_fee
                else max_covered_by_fee
            )
        ...
        for cumulative_fee in range(0, 2000):
            ...
            assert (
                actual_remain_over_issued_icx
                == abs(icx_issue_amount - over_issued_icx) + cumulative_fee
            )
        ...
        for cumulative_fee in range(0, 1005):
            ...
            assert actual_covered_by_remain == over_issued_icx
            assert (
                actual_remain_over_issued_icx == 0
                if cumulative_fee <= max_covered_by_fee
                else cumulative_fee - max_covered_by_fee
            )
            assert (
    ...
    def test_correct_issue_amount_over_issued_icx_is_less_then_icx_issue_amount(self):
        ...
        (
            covered_icx_by_fee,
            covered_icx_by_remain,
            remain_over_issued_iscore,
            corrected_icx_issue_amount,
        ) = Regulator._correct_issue_amount(
            over_issued_i_score, icx_issue_amount, prev_block_cumulative_fee
        )
        ...
        assert (
            corrected_icx_issue_amount
            == icx_issue_amount
            - over_issued_i_score // ISCORE_EXCHANGE_RATE
    ...
    def test_correct_issue_amount_on_calc_period_when_first_calc_period(self):
        ...
        (
            ...
            remain_over_issued_iscore,
            corrected_icx_issue_amount,
        ) = Regulator._correct_issue_amount_on_calc_period(
    ...
    def test_correct_issue_amount_on_calc_period_prev_icx_is_more_than_prev_i_score(
        ...
        expected_diff_without_fee = (
            prev_calc_period_issued_icx * ISCORE_EXCHANGE_RATE
            - prev_calc_period_issued_iscore
            + over_issued_i_score
        )
        ...
        assert (
            corrected_icx_issue_amount
            == icx_issue_amount - expected_covered_by_remain - prev_block_cumulative_fee
        )
        assert (
            remain_over_issued_iscore
            == expected_diff_without_fee
            - expected_covered_by_remain * ISCORE_EXCHANGE_RATE
        )
        ...
        assert (
            remain_over_issued_iscore
            == expected_diff_without_fee - covered_icx_by_remain * ISCORE_EXCHANGE_RATE
        )
    ...
    def test_correct_issue_amount_on_calc_period_prev_icx_is_less_than_prev_i_score_additional_issuing(
        ...
        expected_covered_by_remain = -(
            expected_diff_without_fee // -ISCORE_EXCHANGE_RATE
        )
        assert covered_icx_by_fee == prev_block_cumulative_fee
        assert covered_icx_by_remain == expected_covered_by_remain
        assert corrected_icx_issue_amount == icx_issue_amount + (
    ...
    def test_set_corrected_issue_data_with_in_period(self, mocked_context_storage):
        # create dummy block
        block_height = 5
        ...
        rv = RegulatorVariable(
            current_calc_preiod_issued_icx,
            prev_calc_period_issued_icx,
            over_issued_i_score,
        )
        cumulative_fee = 0
    ...
    @patch("iconservice.iconscore.icon_score_context.IconScoreContext.storage")
    def test_set_corrected_issue_data_end_of_period(
    ...
    def test_set_negative_value_to_prev_calc_preiod_issued_icx(
        ...
        assert (
            regulator._regulator_variable.prev_calc_period_issued_icx
            == issue_amount + current_calc_preiod_issued_icx
        )
