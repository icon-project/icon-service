...
class IconBuiltinScoreLoader(object):
    ...
    @staticmethod
    def load_builtin_scores(
        context: "IconScoreContext", builtin_score_owner: "Address"
    ):
        for score_name, value in BUILTIN_SCORE_ADDRESS_MAPPER.items():
            ...
            if IconScoreContextUtil.is_score_active(context, score_address):
                IconBuiltinScoreLoader._load_score(
                    context, score_address, builtin_score_owner
                )
            else:
