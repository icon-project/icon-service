...
class Term(object):
    ...
    def update(self, lazy_preps: OrderedDict):
        """Update term with the preps which has been invalid on this block
        DO NOT USE it when this term is ended

        :param lazy_preps: P-Reps which are imposed penalty on
        :return:
        """
        self._check_access_permission()

        for prep in lazy_preps:
            if prep.is_suspended():
                self._suspended_preps.append(prep.address)
