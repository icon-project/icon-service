...
def _start_process(conf: "IconConfig"):
    ...
    if is_foreground:
        ...
        subprocess.Popen(
            [sys.executable, "-m", python_module_string, *custom_argv], close_fds=True
        )
    Logger.info("start_process() end")


async def stop_process(conf: "IconConfig"):
    icon_score_queue_name = _make_icon_score_queue_name(
        conf[ConfigKey.CHANNEL], conf[ConfigKey.AMQP_KEY]
    )
    stub = await _create_icon_score_stub(
        conf[ConfigKey.AMQP_TARGET], icon_score_queue_name
    )
...
async def _create_icon_score_stub(
    amqp_target: str, icon_score_queue_name: str
) -> "IconScoreInnerStub":
    from .icon_inner_service import IconScoreInnerStub
