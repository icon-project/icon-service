...
class IconScoreContext(ABC):
    ...
    def __init__(
        self, context_type: "IconScoreContextType" = IconScoreContextType.QUERY
    ) -> None:
        """Constructor
        ...
        self.fee_sharing_proportion = (
            0  # The proportion of fee by SCORE in percent (0-100)
        )
    ...
    def set_decentralize_trigger(cls, decentralize_trigger: float):
        ...
        if not 1.0 > decentralize_trigger >= 0:
            raise FatalException(
                f"Invalid min delegation percent for decentralize: {decentralize_trigger}."
    ...
    def readonly(self):
        return (
            self.type == IconScoreContextType.QUERY
            or self.func_type == IconScoreFuncType.READONLY
        )
    ...
    def is_revision_changed(self, target_rev: int) -> bool:
        ...
        return (
            old.revision_code != new.revision_code and new.revision_code == target_rev
        )
    ...
    def is_the_first_block_on_decentralization(self) -> bool:
        return (
            self._term.sequence == 0
            and self.block.height == self._term.start_block_height
        )
    ...
    def _update_prep_address_converter(self, dirty_prep: "PRep"):
        if not self._preps.contains(dirty_prep.address, active_prep_only=False):
            # registerPRep
            self._prep_address_converter.add_node_address(
                node=dirty_prep.node_address, prep=dirty_prep.address
            )
        elif dirty_prep.is_flags_on(PRepFlag.NODE_ADDRESS):
            # setPRep
            old_prep = self._preps.get_by_address(dirty_prep.address)
            self._prep_address_converter.replace_node_address(
                node=dirty_prep.node_address,
            ...
            self._prep_address_converter.delete_node_address(
                node=dirty_prep.node_address
            )
    ...
    def put_dirty_prep(self, prep: "PRep"):
        ...
        if (
            not prep.is_dirty()
            and self.revision >= Revision.OPTIMIZE_DIRTY_PREP_UPDATE.value
        ):
            Logger.info(
                tag=self.TAG,
                msg=f"No need to update an unchanged P-Rep: revision={self.revision}",
            )
            # Logger.debug(tag=self.TAG, msg="put_dirty_prep() end")
            return
    ...
    def set_step_counter(self, step_limit: Optional[int] = None):
        ...
        self.step_counter = IconScoreStepCounter(
            step_price=self._inv_container.step_price,
...
class IconScoreContextFactory(object):
    ...
    @classmethod
    def _create_context(
        cls, context_type: "IconScoreContextType"
    ) -> "IconScoreContext":
    ...
    def _set_context_attributes_for_processing_tx(cls, context: "IconScoreContext"):
        if context.type in (
            ...
            context._prep_address_converter = (
                context.engine.prep.prep_address_converter.copy()
            )
        else:
            # Readonly
