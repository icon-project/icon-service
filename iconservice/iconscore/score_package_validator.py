...
class ScorePackageValidator(object):
    ...
    @classmethod
    def execute(
        cls, whitelist_table: dict, pkg_root_path: str, pkg_root_package: str
    ) -> callable:
    ...
    def _validate_import_from_code(cls, code):
        ...
        for code_index in range(OPCODE_HEADER_END_INDEX, length_byte_code_list, 2):
            key = byte_code_list[code_index]
            if IMPORT_NAME == key:
                cls._validate_import(
                    code_index, byte_code_list, code.co_names, code.co_consts
                )
    ...
    def _validate_import(
        cls, current_index: int, byte_code_list: list, co_names: tuple, co_consts: tuple
    ):
        """ example
        20 LOAD_CONST               0 (0)
        ...
        if from_list is None:
            ...
            if IMPORT_STAR == next_op_code_key:
                ...
                for import_from in from_list:
                    if (
                        "*" not in cls.WHITELIST_IMPORT[import_name]
                        and import_from not in cls.WHITELIST_IMPORT[import_name]
                    ):
                        raise IllegalFormatException(
                            f"Invalid import name: {import_name}"
                        )
                    elif (
                        import_name in BASE_PACKAGE
                        and import_from not in cls.ICONSERVICE_WHITELIST
                    ):
                        raise IllegalFormatException(
                            f"Invalid permission import: {import_from}"
                        )
