...
class InternalCall(object):
    ...
    @classmethod
    def icx_get_balance(cls, context: 'IconScoreContext', address: 'Address') -> int:
        return cls.icx_engine.get_balance(context, address)

    @classmethod
    def icx_transfer_call(cls,
        ...
        """
        cls._call(context, addr_from, addr_to, STR_FALLBACK, amount, None, None)

    @classmethod
    def other_external_call(cls,
                            context: 'IconScoreContext',
                            addr_from: 'Address',
                            addr_to: 'Address',
                            func_name: str,
        ...
        return cls._call(context, addr_from, addr_to, func_name, amount, arg_params, kw_params)
    ...
    def _call(cls,
        ...
        try:
            cls._make_trace(context, addr_from, addr_to, func_name, amount, arg_params, kw_params)

            context.step_counter.apply_step(StepType.CONTRACT_CALL, 1)

            cls.icx_engine.transfer(context, addr_from, addr_to, amount)

            if amount > 0:
                cls.emit_event_log_for_icx_transfer(context, addr_from, addr_to, amount)

            if addr_to.is_contract:
                return cls._other_score_call(context, addr_from, addr_to, func_name, amount, arg_params, kw_params)
            return None
    ...
    @classmethod
    def emit_event_log_for_icx_transfer(cls,
                                        context: 'IconScoreContext',
                                        from_: 'Address',
                                        to: 'Address',
                                        value: int) -> None:
