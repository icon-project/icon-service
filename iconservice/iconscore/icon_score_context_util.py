...
class IconScoreContextUtil(object):
    ...
    @staticmethod
    def get_builtin_score(
        context: "IconScoreContext", address: "Address"
    ) -> "IconScoreBase":
    ...
    def get_icon_score(
        ...
        :return:
        """
        score_info: "IconScoreInfo" = IconScoreContextUtil.get_score_info(
            context, address
        )
    ...
    def get_score_info(
        ...
        deploy_info: "IconScoreDeployInfo" = IconScoreContextUtil.get_deploy_info(
            context, address
        )
        ...
        if score_info is None:
            score_info: "IconScoreInfo" = IconScoreContextUtil.create_score_info(
                context, address, current_tx_hash
            )
    ...
    def create_score_info(
        ...
        tx_hash: bytes,
        score_db: "IconScoreDatabase" = None,
    ) -> "IconScoreInfo":
    ...
    @staticmethod
    def validate_score_package(
        context: "IconScoreContext", address: "Address", tx_hash: bytes
    ) -> None:

        if not IconScoreContextUtil.is_service_flag_on(
            context, IconServiceFlag.SCORE_PACKAGE_VALIDATOR
        ):
            return

        score_deploy_path: str = get_score_deploy_path(
            context.score_root_path, address, tx_hash
        )
        score_package_name: str = get_package_name_by_address_and_tx_hash(
            address, tx_hash
        )
        import_whitelist: dict = context.inv_container.import_white_list

        ScorePackageValidator.execute(
            import_whitelist, score_deploy_path, score_package_name
        )

    @staticmethod
    def validate_score_blacklist(
        context: "IconScoreContext", score_address: "Address"
    ) -> None:
    ...
    @staticmethod
    def is_service_flag_on(
        context: "IconScoreContext", flag: "IconServiceFlag"
    ) -> bool:
    ...
    @staticmethod
    def get_score_address_by_tx_hash(
        context: "IconScoreContext", tx_hash: bytes
    ) -> Optional["Address"]:
        warnings.warn("legacy function don't use.", DeprecationWarning, stacklevel=2)
    ...
    def get_deploy_tx_params(
        context: "IconScoreContext", tx_hash: bytes
    ) -> Optional["IconScoreDeployTXParams"]:
