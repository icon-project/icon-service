...
class Storage(StorageBase):
    ...
    def get_regulator_variable(self, context: "IconScoreContext"):
        regulator_variable: Optional[bytes] = self._db.get(
            context, self._REGULATOR_VARIABLE_KEY
        )
        if regulator_variable:
            return RegulatorVariable.from_bytes(regulator_variable)
        return RegulatorVariable(
            current_calc_period_issued_icx=0,
            prev_calc_period_issued_icx=-1,
            over_issued_iscore=0,
        )

    def put_regulator_variable(
        self, context: "IconScoreContext", rv: "RegulatorVariable"
    ):
...
class RegulatorVariable:
    ...
    def __str__(self):
        return (
            f"Current calc period issued ICX: {self.current_calc_period_issued_icx} "
