...
class Regulator:
    ...
    def _set_corrected_issue_data(self, context: "IconScoreContext", issue_amount: int):
        ...
        if end_block_height_of_calc == context.block.height:
            ...
            if regulator_variable.prev_calc_period_issued_icx == -1:
                (
                    regulator_variable.prev_calc_period_issued_icx,
                    prev_calc_period_issued_iscore,
                ) = (0, 0)
            (
                covered_icx_by_fee,
            ...
            ) = self._correct_issue_amount_on_calc_period(
                regulator_variable.prev_calc_period_issued_icx,
                prev_calc_period_issued_iscore,
            ...
            regulator_variable.prev_calc_period_issued_icx = (
                current_calc_period_total_issued_icx
            )
        ...
        Logger.info(
            ...
            f"Regulator variable: {self._regulator_variable}",
            IISS_LOG_TAG,
        )
    ...
    @classmethod
    def _reflect_difference_in_issuing(
        cls, icx_issue_amount: int, over_issued_icx: int, prev_block_cumulative_fee: int
        ...
        (
            ...
            corrected_icx_issue_amount,
        ) = cls._calculate_over_issued_icx(icx_issue_amount, over_issued_icx)

        (
            covered_icx_by_fee,
            remain_over_issued_icx,
            corrected_icx_issue_amount,
        ) = cls._calculate_prev_block_cumulative_fee(
            remain_over_issued_icx,
            corrected_icx_issue_amount,
            prev_block_cumulative_fee,
        )
    ...
    @classmethod
    def _calculate_over_issued_icx(
        cls, icx_issue_amount: int, over_issued_icx: int
    ) -> Tuple[int, int, int]:
    ...
    @classmethod
    def _calculate_prev_block_cumulative_fee(
        cls,
        remain_over_issued_icx: int,
        corrected_issue_amount: int,
        prev_block_cumulative_fee: int,
    ) -> Tuple[int, int, int]:
        ...
        if corrected_issue_amount >= 0:
            ...
            remain_over_issued_icx = remain_over_issued_icx + abs(
    ...
    @classmethod
    def _correct_issue_amount_on_calc_period(
        cls,
        ...
        prev_calc_over_issued_iscore: int = prev_calc_period_issued_icx * ISCORE_EXCHANGE_RATE - prev_calc_period_issued_iscore
        total_over_issued_iscore: int = prev_calc_over_issued_iscore + remain_over_issued_iscore
        over_issued_icx, over_issued_iscore = cls._separate_icx_and_iscore(
            total_over_issued_iscore
        )
        ...
        (
            ...
            remain_over_issued_icx,
            icx_issue_amount,
        ) = cls._reflect_difference_in_issuing(
            icx_issue_amount, over_issued_icx, prev_block_cumulative_fee
        )
