...
class Governance(IconScoreBase):
    ...
    def on_install(
        ...
        maxQueryStepLimit: int = 0x780000,
    ) -> None:
        super().on_install()
    ...
    def getScoreStatus(self, address: Address) -> dict:
        ...
        if count2 > 0:
            ...
            if next_tx_hash is not None and next_tx_hash != status[DEPLOY_TX_HASH]:
        ...
        if build_initial_status:
            status = {STATUS: STATUS_PENDING, DEPLOY_TX_HASH: next_tx_hash}
            result[NEXT] = status
        return result
    ...
    def rejectScore(self, txHash: bytes, reason: str):
        ...
        if score_address is None:
            self.revert("Invalid txHash")
        Logger.debug(
            f'rejectScore: score_address = "{score_address}", reason = {reason}', TAG
        )
        # check next: it should be 'pending'
        result = self.getScoreStatus(score_address)
        try:
