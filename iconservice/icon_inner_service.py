...
class IconScoreInnerTask(object):
    ...
    async def hello(self):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.INVOKE):
            ...
            ret = await loop.run_in_executor(
                self._thread_pool[THREAD_INVOKE], self._hello
            )
        else:
            ret = self._hello()
    ...
    async def invoke(self, request: dict) -> dict:
        ...
        if self._is_thread_flag_on(EnableThreadFlag.INVOKE):
            loop = asyncio.get_event_loop()
            ret: dict = await loop.run_in_executor(
                self._thread_pool[THREAD_INVOKE], self._invoke, request
            )
        else:
    ...
    def _invoke(self, request: dict):
        ...
        try:
            ...
            if convert_tx_result_to_dict:
                ...
                # old version
                convert_tx_results = {
                    bytes.hex(tx_result.tx_hash): tx_result.to_dict(to_camel_case)
        ...
        Logger.info(
            tag=_TAG,
            msg=f"INVOKE Response: {json.dumps(response, cls=BytesToHexJSONEncoder)}",
        )
        return response
    ...
    async def query(self, request: dict):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.QUERY):
            ...
            return await loop.run_in_executor(
                self._thread_pool[THREAD_QUERY], self._query, request
            )
        else:
    ...
    async def call(self, request: dict):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.QUERY):
            loop = asyncio.get_event_loop()
            ret = await loop.run_in_executor(
                self._thread_pool[THREAD_QUERY], self._call, request
            )
        else:
    ...
    async def write_precommit_state(self, request: dict):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.INVOKE):
            loop = asyncio.get_event_loop()
            ret = await loop.run_in_executor(
                self._thread_pool[THREAD_INVOKE], self._write_precommit_state, request
            )
        else:
    ...
    async def remove_precommit_state(self, request: dict):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.INVOKE):
            loop = asyncio.get_event_loop()
            ret = await loop.run_in_executor(
                self._thread_pool[THREAD_INVOKE], self._remove_precommit_state, request
            )
        else:
    ...
    def _remove_precommit_state(self, request: dict):
        ...
        try:
            ...
            Logger.info(
                tag=_TAG,
                msg=f"REMOVE_PRECOMMIT_STATE: BH={block_height} "
                f"instant_block_hash={bytes_to_hex(instant_block_hash)}",
            )
    ...
    async def validate_transaction(self, request: dict):
        ...
        if self._is_thread_flag_on(EnableThreadFlag.VALIDATE):
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(
                self._thread_pool[THREAD_VALIDATE], self._validate_transaction, request
            )
        else:
...
class IconScoreInnerService(MessageQueueService[IconScoreInnerTask]):
    ...
    def clean_close(self):
        Logger.debug(
            tag=_TAG,
            msg="icon service will be closed while open the icon service engine. "
