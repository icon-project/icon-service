...
class Storage(StorageBase):
    ...
    def open(self, context: "IconScoreContext", iiss_meta_data: dict, calc_period: int):
        self.check_config_before_init(iiss_meta_data)

        meta_data: Optional["IISSMetaData"] = self.get_meta_data(context)
        if meta_data is None:
            meta_data = IISSMetaData(
                reward_min=iiss_meta_data[ConfigKey.REWARD_MIN],
        ...
        for k, v in iiss_variable.items():
            if k.startswith("reward") and not 0 < v <= IISS_MAX_REWARD_RATE:
                raise FatalException(
                    f"Out of reward rate range: 0 < {v} <= {IISS_MAX_REWARD_RATE}"
                )
    ...
    def put_end_block_height_of_calc(
        self, context: "IconScoreContext", calc_block_height: int
    ):
        version = 0
    ...
    def get_end_block_height_of_calc(
...
class RewardRate:
    ...
    def __init__(self, reward_prep: Optional[int]):
    ...
    def to_bytes(self):
        data: list = [self._VERSION, self.reward_prep]
